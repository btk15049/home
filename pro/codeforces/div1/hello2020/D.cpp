...
int main() {
    ...
    for (int i : range(n)) {
        ...
        latte[malta++] = ea[i];
        latte[malta++] = eb[i];
        c[i]           = (((LL)r.next_int()) << 32) + r.next_int();
    }
    sort(latte, latte + malta);
    malta = unique(latte, latte + malta) - latte;
    for (int i : range(n)) {
        sa[i] = lower_bound(latte, latte + malta, sa[i]) - latte + 2;
        sb[i] = lower_bound(latte, latte + malta, sb[i]) - latte + 2;
        ea[i] = lower_bound(latte, latte + malta, ea[i]) - latte + 2;
        eb[i] = lower_bound(latte, latte + malta, eb[i]) - latte + 2;
    }
    ...
    for (int i : range(n)) {
        ...
        LL setA = 0;
        LL setB = 0;
        setA ^= bgA.sum(sa[i] - 1) ^ edA.sum(sa[i] - 1);
        setA ^= bgA.sum(ea[i]) ^ bgA.sum(sa[i] - 1);
        setB ^= bgB.sum(sb[i] - 1) ^ edB.sum(sb[i] - 1);
        setB ^= bgB.sum(eb[i]) ^ bgB.sum(sb[i] - 1);
