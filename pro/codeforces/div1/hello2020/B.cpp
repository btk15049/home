// http://codeforces.com/contest/1284/problem/B
#define CIN_ONLY
...
int main() {
    ...
    for (int i : range(n)) {
        bt[i] = *min_element(ALL(v[i]));
        tp[i] = *max_element(ALL(v[i]));
        for (int j : range(1, v[i].size())) {
            if (v[i][j - 1] < v[i][j]) ok[i] = 1;
        }
        if (ok[i] == 0) {
            s_bt.push_back(bt[i]);
            s_tp.push_back(tp[i]);
    ...
    for (int i : range(n)) {
        if (ok[i] == 0) {
            ret += ok_cnt + distance(upper_bound(ALL(s_tp), bt[i]), s_tp.end());
        }
        else {
            ret += n;
        }
