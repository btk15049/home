// http://codeforces.com/contest/1286/problem/C1
#define CIN_ONLY
...
int main() {
    ...
    int n;
    cin >> n;
    vector<map<string, int>> before(n + 1);
    vector<map<string, int>> after(n + 1);
    if (n == 1) {
        auto v = query(0, 1);
        out(v.front());
    }
        ...
        // 4 a a b c aa ab bc aab abc aabc a b c ab bc abc
