// http://codeforces.com/contest/1278/problem/F
#define CIN_ONLY
...
namespace factorial {
    ModInt naivePermutation(int n, int k) {
        if (k < 0 || k > n) return ModInt::raw(0);
        ModInt ret = 1;
        for (int i = n; i > n - k; i--) {
            ret *= ModInt(i);
    ...
    ModInt naiveCombination(int n, int k) {
        if (k < 0 || k > n) return ModInt::raw(0);
        if (!is_build) build();
        return naivePermutation(n, k) * inverse_factorial[k];
    }
} // namespace factorial
...
int main() {
    ...
    for (int i : range(1, k + 1)) {
        auto a = factorial::naiveCombination(n, i); // カード位置を固定
        auto b = inv(ModInt(m)) ^ i; // 選ばれたカードがジョーカーになる確率
        /* 選ばれたカードを全種類使うような経路の本数 */
        memo[i] = ModInt(i) ^ k;
        for (int j : range(1, i)) {
            memo[i] -= factorial::combination(i, j) * memo[j];
        }
        ret += a * b * memo[i];
    }
    cout << *ret << endl;
