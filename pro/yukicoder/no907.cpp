...
// 積分
// 定数部は0
vector<ModInt> up(vector<ModInt> v) {
    for (int i : range(v.size())) {
        v[i] = v[i] / (i + 1);
    }
    reverse(ALL(v));
    v.push_back(ModInt(0));
    reverse(ALL(v));
    return v;
}

// 多項式にxを代入
ModInt assign(vector<ModInt> v, ModInt x) {
    v          = up(v);
    ModInt xx  = 1;
    ModInt ret = 0;
    for (auto& it : v) {
...
vector<ModInt> ad(vector<ModInt> x, vector<ModInt> y) {
...
ostream& operator<<(ostream& os, vector<ModInt> a) {
    os << "{";
    for (auto& it : a) {
        os << *it << ",";
    }
...
int main() {
    ...
    for (int i : range(N)) {
        AI[i] = lower_bound(latte, latte + malta, A[i]) - latte;
        BI[i] = lower_bound(latte, latte + malta, B[i]) - latte;
    }
    vector<vector<ModInt>> dp(malta - 1);
    {
        const int bg = AI[0];
        const int ed = BI[0];
        ModInt p     = ModInt(1) / (B[0] - A[0]);
    ...
    // for (int j : range(malta - 1)) {
    //    cerr << j << " " << dp[j] << endl;
    //}

    for (int i : range(1, N)) {
        ModInt p     = ModInt(1) / (B[i] - A[i]);
        const int bg = AI[i];
        const int ed = BI[i];
        vector<vector<ModInt>> nxt(malta - 1);
        // 増える
        if (i % 2 == 1) {
            ModInt C = 0;
            for (int j : range(malta - 1)) {
                ...
                if (bg <= j && j + 1 <= ed) {
                    nxt[j] = up(dp[j]);
                    nxt[j][0] -= assign(dp[j], ModInt(latte[j]));
                    nxt[j][0] += C;
                    for (auto& it : nxt[j]) it *= p;
                }
                else {
                    nxt[j] = {ModInt(0)};
                }
                C += assign(dp[j], ModInt(latte[j + 1]))
                     - assign(dp[j], ModInt(latte[j]));
            }
        }
        // 減る
        else {
            ...
            for (int j : !range(malta - 1)) {
                if (bg <= j && j + 1 <= ed) {
                    nxt[j] = up(dp[j]);
                    for (auto& it : nxt[j]) it = -it;
                    nxt[j][0] += assign(dp[j], ModInt(latte[j + 1]));
                    nxt[j][0] += C;
                    for (auto& it : nxt[j]) it *= p;
                }
                else {
    ...
    ModInt ret = 0;
    for (int i : range(malta - 1)) {
        ret = ret + assign(dp[i], ModInt(latte[i + 1]))
              - assign(dp[i], ModInt(latte[i]));
    }
    cout << *ret << endl;
