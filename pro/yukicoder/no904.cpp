...
int main() {
    ...
    for (int i : range(N - 1)) cin >> A[i] >> B[i];
    int ret       = 0;
    const int as  = accumulate(ALL(A), 0);
    const int bs  = accumulate(ALL(B), 0);
    const int a_b = as - bs;

    for (const int a : range(0, 37 * 10000 * 2)) {
        const int b = a + a_b;
        if (b < 0) continue;
        A.push_back(a);
        B.push_back(b);
        bool ok = true;
        for (int i : range(N)) {
            if (A[i] > bs + B.back() - B[i]) ok = false;
            if (B[i] > as + A.back() - A[i]) ok = false;
        }
        if (ok) ret++;
        A.pop_back();
        B.pop_back();
