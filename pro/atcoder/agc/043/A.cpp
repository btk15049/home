...
int main() {
    ...
    vector<string> S(r);
    cin >> S;
    auto ret = nestingVector<int>(r, c);
    if (S[0][0] == '#' || S.back().back() == '#') ret[0][0] += 2;
    for (int i : range(r)) {
        for (int j : range(c)) {
            auto &v = ret[i][j];
            if (i + j != 0) v = 1000;
            if (i > 0) {
                chmin(v, ret[i - 1][j] + (S[i][j] != S[i - 1][j] ? 1 : 0));
            }
            if (j > 0) {
                chmin(v, ret[i][j - 1] + (S[i][j] != S[i][j - 1] ? 1 : 0));
            }
        }
        // cerr << join(ret[i], ' ') << endl;
    }

    cout << ret.back().back() / 2 << endl;
    return 0;
}