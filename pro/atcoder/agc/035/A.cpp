...
int main() {
    ...
    for (auto &it : cnt) x.push_back(it);
    sort(ALL(x), [](pair<int, int> p, pair<int, int> q) {
        return p.second > q.second;
    });
    if (cnt.count(0) > 0 && cnt.size() == 2) {
        if (cnt[0] * 3 == N) {
            cout << ret[true] << endl;
            return 0;
        }
