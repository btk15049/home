...
void verify(LL a, LL b) {
    ...
    vector<int> p;
    for (int i : range(n)) {
        p.push_back(abs(xx - x[i]) + abs(yy - y[i]));
    }
    for (int i : range(n - 1)) {
        if (p[i] != p[i + 1]) return;
    }
...
int main() {
    ...
    int xpy_b = *min_element(xpy, xpy + n);
    int xpy_t = *max_element(xpy, xpy + n);
    int xmy_b = *min_element(xmy, xmy + n);
    int xmy_t = *max_element(xmy, xmy + n);


    int d = max(xpy_t - xpy_b, xmy_t - xmy_b) / 2;
    verify(xpy_t - d, xmy_t - d);
    verify(xpy_t - d, xmy_b + d);
    verify(xpy_b + d, xmy_t - d);
    verify(xpy_b + d, xmy_b + d);
