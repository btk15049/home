...
ModInt dp[1 << 20];
bool vis[1 << 20];
E must_edge,extra_edge,mask;
string tos(int bit){
    return V(bit).to_string().substr(20-N);
}
void recur(int bit,E inner_must_edge,E middle_must_edge,
E outer_must_edge,E inner_extra_edge,E middle_extra_edge,E outer_extra_edge) {
    if (vis[bit]) return;
    vis[bit] = true;
    if (bit == (1 << N) - 1) {
        dp[bit] = 1;
    ...
    for (int i : range(N - 1)) {
        if (inout[i]) {
            int in = a[i], out = b[i];
            if (!inner_v[in]) swap(in, out);
            V nxt_inner_v     = inner_v;
            nxt_inner_v[out]  = true;
            const int nxt_bit = (int)(nxt_inner_v.to_ulong());

            E new_added_inner_edge  = d[out] & inout;
            E new_added_inout_edge = d[out] ^ new_added_inner_edge;
            E new_added_inout_must_edge = d[out] & must_edge;
            new_added_inner_edge[i] = false;
            ModInt C = 1;
            C = C * factorial::permutation(outer.count(),new_added_inner_edge.count());
            const int in_must_edge = (inner | inout).count();
            C = C * factorial::combination((N-1)-in_must_edge,new_added_inout_must_edge.count());
            recur(nxt_bit);
            DEBUG if(N<10) cerr << tos(bit) << ">" << tos(nxt_bit)  << " ";
            DEBUG if(N<10) cerr << outer.count() << " " << new_added_inner_edge.count() << endl;
            dp[bit] = dp[bit] + dp[nxt_bit] * C;
        }
    }
}
int main() {
    ...
    for(int i:range(N-1))must_edge[i]=true;
    for(int i:range(N-1,M))extra_edge[i]=true;
  //recur(1);
    cout << *dp[1] << endl;
    return 0;
}