...
int main() {
    ...
    for (int l : L) {
        ...
        for (int i : range(N)) {
            ...
            V dp_i_sum(X + 2);
            for (int j : range(X + 1)) {
                dp_sum[j + 1]   = dp_sum[j] + dp[i][j];
                dp_i_sum[j + 1] = dp_i_sum[j] + (j + 1) * dp[i][j];
            }

            for (int j : range(l, X + 1)) {
                // 新たに追加
                nx[i + 1][j] += dp[i][j - l];
                if (i >= 1) {
                    // 既存のどこかに隠す
                    nx[i][j] += dp[i][j] * (i + j - i * l);
                    // 既存の一つを伸ばす
                    nx[i][j] +=
                        ModInt(2) * i * (dp_sum[j] - dp_sum[j - (l - 1)]);
                }
                if (i >= 2) {
                    // 既存の二つを併合する
                    // {i * (i-1)} *
                    //{dp[j - (l - 2)] * 1 + dp[j - (l - 1)] * 2 + ...+ dp[j- 1]
                    //* (l - 2) + dp[j] * (l - 1)}
                    ModInt y =
                        (dp_i_sum[j + 1] - dp_i_sum[j - (l - 2)])
                        - (j - (l - 2)) * (dp_sum[j + 1] - dp_sum[j - (l - 2)]);
                    nx[i - 1][j] += ModInt(i * (i - 1)) * y;
                }
            }
        }
        swap(dp, nx);
    ...
    for (int k : range(1, N + 1)) {
        ret += dp[k].back() * factorial::permutation(k, k);
    }
    cout << *ret << endl;
    return 0;
}