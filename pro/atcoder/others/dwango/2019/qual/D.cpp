...
int main() {
    ...
    for (int i : range(N - 1)) {
        int target = *s.begin();
        if (ret.size() != 0u && a[ret.back()] == target) {
            target = *s.upper_bound(target);
        }
    ...
    }
    {
        ...
        s.erase(target);
        auto it = ret.end();
        while (it != ret.begin() && a[*prev(it)] == target) --it;
        ret.insert(it, target);
        //for (int i : range(N)) {
        //    cerr << ret[i] + 1 << sp[i];
        //}
        // cerr << target + 1 << endl;

        sort(next(find(ALL(ret), target)), ret.end());
        it = find(ALL(ret), target);
        // for (int i : range(N)) {
        //    cerr << ret[i] + 1 << sp[i];
        //}
        // cerr << target + 1 << endl;
        if (next(it) != ret.end() && *next(it) == a[target]) {
            if (next(next(it)) != ret.end()) {
                swap(*next(it), *next(next(it)));
            }
            else {
                // cerr << "a" << endl;
                it = ret.end();
                ...
                do {
                    bool ok = true;
                    if (ret.size() > 3) {
                        ok = (ok && a[ret[N - 4]] != ret[N - 3]);
                    }
                    ok = ok && (a[ret[N - 3]] != ret[N - 2]);
                    ok = ok && (a[ret[N - 2]] != ret[N - 1]);
                    if (ok) break;
                } while (next_permutation(it, ret.end()));
            }
        }
    }
    for (int i : range(N)) {
