...
int main() {
    ...
    vector<vector<pair<int, int>>> p(malta);

    for (int i : range(n)) {
        int id = lower_bound(latte, latte + malta, x[i] - l[i]) - latte;
        p[id].push_back(
            {id, lower_bound(latte, latte + malta, x[i] + l[i]) - latte});
    }
    vector<int> dp(malta);
    for (int i : range(malta)) {
        for (auto s : p[i]) {
            dp[s.second] = max(dp[s.second], dp[i] + 1);
        }
        chmax(dp[i + 1], dp[i]);
    }
    cout << dp.back() << endl;
