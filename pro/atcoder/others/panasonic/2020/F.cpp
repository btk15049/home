...
int main() {
    ...
    for (int i : range(q)) {
        ...
        for (int i : range(33)) {
            ap.push_back({ar % 3, ac % 3});
            ar /= 3;
            ac /= 3;
            bp.push_back({br % 3, bc % 3});
            br /= 3;
            bc /= 3;
        }
        int k = 32;
        ...
        cerr << k << endl;
        LL fk = f(k);
        if (ap[k].first == 1 && ap[k].first == bp[k].first) {
            a %= fk;
            c %= fk;
            cout << abs(b - d)
                        + multi_min(a + c + 2, (fk - a) + (fk - c),
                                    (fk - a) + c + 1, a + 1 + (fk - c))

                 << endl;
        }
        else if (ap[k].second == 1 && ap[k].second == bp[k].second) {
            b %= fk;
            d %= fk;
            cout << abs(a - c)
                        + multi_min(b + d + 2, (fk - b) + (fk - d),
                                    (fk - b) + d + 1, b + 1 + (fk - d))
