...
struct BIT {
    ...
    T* bit;
    int sz;
    BIT(int n = 0) : bit(BIT_::get(n + 10)), sz(n) {
        fill(bit, bit + n + 10, 0);
    }
    T sum(int i) {
        T s = 0;
...
int main() {
    ...
    for (int i : range(n)) {
        for (int len : range(1, n)) {
            if (i + len > n) continue;
            int ok = 1;
            for (int j : range(len)) {
                if (s[i + j] != s[i + len + j]) ok = 0;
            }
            bits[i].add(2 * len, ok);
        }
    }
    ...
    for (int i : range(1, n)) {
        for (int len : range(1, n)) {
            if (!(i + len + 3 <= n - len)) continue;
            int ok = 1;
            for (int j : range(len)) {
                if (s[i + j] != s[n - len + j]) ok = 0;
            }
            // cerr << i << " " << len << " " << ok << " "
            //<< bits[i + len].sum((n - len) - (i + len) - 1) << endl;
            ret += ok * bits[i + len].sum((n - len) - (i + len) - 1);
        }
    }
    cout << ret << endl;
