...
int N;
int K[212345];
vector<ModInt> A[212345];
map<pair<int,int>,ModInt> memo;
int sid[212345];
ModInt S[212345/threshold][threshold][threshold];

ModInt same(int a,int b){
    ModInt ret=0;
    for(int i:range(K[b])){
        ret=ret+A[a][i%K[a]]*A[b][i%K[b]];
    }
    return ret;
}
ModInt diff(int a,int b){
    ModInt ret=0;
    for(int i:range(K[a])){
        ret=ret+A[a][i]*S[sid[b]][K[a]][i];
    }
    return ret;
}
void build(){
    ...
    for(int i:range(N)){
        if(K[i]>=threshold){
            sid[i]=T;
            for(int j:range(1,threshold)){
                for(int k:range(K[i])){
                    S[T][j][k%j]=S[T][j][k%j]+A[i][k];
                }
            }
            T++;
        }
...
ModInt solve(int a,int b){
    if(K[a]>K[b])swap(a,b);
    if(memo.count({a,b}))return memo[{a,b}];
    ModInt ret=0;
    if(K[b]<threshold || K[a]>=threshold){
        ret = same(a,b);
    }
    else{
        ret = diff(a,b);
