...
struct BIT {
    ...
    T* bit;
    int sz;
    BIT(int n) : bit(BIT_::get(n + 10)), sz(n) { fill(bit, bit + n + 10, INF); }
    T min(int i) {
        T s = INF;
        while (i > 0) {
            chmin(s, bit[i]);
...
int main() {
    /* write here */
    map<int, vector<vector<LL>>> in;
    int N;
    cin >> N;
    for (int i : range(N)) {
        LL x, y, z;
        cin >> x >> y >> z;
        in[x].push_back(vector<LL>{y, z, i});
        latte[malta++] = y;
        latte[malta++] = z;
    }
    ...
    BIT bit(malta + 10);
    for (auto& p : in) {
        for (auto& yzi : p.second) {
            LL y   = lower_bound(latte, latte + malta, yzi[0]) - latte;
            LL z   = lower_bound(latte, latte + malta, yzi[1]) - latte;
            LL i   = yzi[2];
            ret[i] = bit.min(y + 1) < z ? 1 : 0;
        }
        for (auto& yzi : p.second) {
            LL y = lower_bound(latte, latte + malta, yzi[0]) - latte;
            LL z = lower_bound(latte, latte + malta, yzi[1]) - latte;
