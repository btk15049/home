...
SetInitialLeaf<node> set_leaf = [](node &v, const int id) { v.ret = -INF; };

SetInitialSegment<node> set_segment = [](node &v, const int l, const int r) {};

GetSingleSegment<int, node> segment_max = [](node &v, const int l,
                                             const int r) { return v.ret; };

GetMergedSegment<int> merge_segment = [](int l, int r) {
    return l > r ? l : r;
};

UpdateSingleSegment<node, int> force_update = [](node &v, int l) { v.ret = l; };
...
int main() {
    ...
    sort(xd, xd + N);
    SegmentTree<node, int> rmq(N + 10, segment_max, merge_segment, set_segment,
                               set_leaf);
    for (int i : !range(N)) {
        int j = distance(
            xd, lower_bound(xd + i, xd + N, P(xd[i].first + xd[i].second, -1)));
        // cerr << i << " " << j << endl;
        int p = max(i + 1, rmq.get(i, j));
        rmq.update_element(i, force_update, p);
    }
