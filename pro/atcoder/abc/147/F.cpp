...
int main() {
    ...
    if (X == 0) {
        cout << (N * (N - 1ll)) / 2 + 1 << endl;
        return 0;
    }
    LL ret = 0;
    map<LL, map<LL, LL>> seg;

    LL b = 1e17;
    for (int i : range(N + 1)) {
        LL mod = i * X % D;
        LL _lb = i * X + (0 + (i - 1) * D) * i / 2;
        LL _ub = i * X + ((N - i) * D + (N - 1) * D) * i / 2;
        DEBUG cerr << _lb << " " << _ub << endl;
        LL lb  = (b + _lb) / D;
        LL ub  = (b + _ub) / D + 1;
        LL see = lb;
        if (seg.count(mod) == 0) {
        ...
        DEBUG cerr << mod << ":";
        // for (auto& it : seg[mod]) {
        //    DEBUG cerr << "{" << it.second << "," << it.first << "}, ";
        //}
        DEBUG cerr << diff << endl;
        ret += diff;
    }
    cout << ret << endl;
    return 0;
