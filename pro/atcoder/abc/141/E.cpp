...
string S;
constexpr int H = 1e9;
using RH1       = RollingHash<9001, 999999941>;
using RH2       = RollingHash<90001, 999999937>;

bool solve(RH1::Table &rh1, RH2::Table &rh2, int len) {
    unordered_map<LL, int> st;
    for (int i : range(N)) {
        if (i + len > N) break;
        LL x = rh1.substr(i, i + len).hash * H + rh2.substr(i, i + len).hash;
        if (st.count(x)) {
            if (st[x] + len <= i) return true;
        }
        else {
            st[x] = i;
        }
