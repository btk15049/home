...
LL K;
void solve(LL x1, LL y, LL x2, vector<LL> &xs, vector<LL> &ys) {
    cerr << x1 << "," << y << "," << x2 << endl;
    while (x1 >= K) {
        xs.push_back(K);
        ys.push_back(0);
...
void solve(LL x1, LL y1, LL x2, LL y2, vector<LL> &xs, vector<LL> &ys) {
...
LL solve(LL X, LL Y, vector<LL> &xs, vector<LL> &ys) {
    // X <= Y
...
int main() {
    ...
    LL ret;
    if (abs(X) <= abs(Y)) {
        ret = solve(abs(X), abs(Y), xs, ys);
    }
    else {
        ret = solve(abs(Y), abs(X), ys, xs);
    }
    if (X < 0) {
        for (auto &it : xs) it = -it;
    }
    if (Y < 0) {
        for (auto &it : ys) it = -it;
    }
