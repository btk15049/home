...
bool operator<(const gus& l, const gus& r) {
    if (l.cnt < r.cnt) return true;
    if (l.cnt > r.cnt) return false;
    return l.lim > r.lim;
}
bool operator!=(const gus& l, const gus& r) {
    return l.cnt != r.cnt || l.lim != r.lim;
}

gus operator+(gus g, const LL c) {
    g.lim -= c;
    if (g.lim < 0) {
        g.lim = L - c;
        g.cnt++;
...
gus operator-(gus g) { return gus(-g.cnt, -g.lim); }


int main() {
    /* write here */
    cin >> N >> M >> L;
    Graph<WeightedEdge<LL>> g(N, M);
    g.resize(N);
    ...
    vector<vector<gus>> dist(N);
    for (int i : range(N)) {
        dist[i] = dijkstra(g, gus(N + 1, 0), i);
    }
    cin >> Q;
    for (int i : range(Q)) {
        int s, t;
        ...
        if (dist[s][t].cnt == N + 1) {
            ...
            cout << dist[s][t].cnt << "\n";
            // cerr << s + 1 << " " << t + 1 << " ";
            // cerr << dist[s][t].cnt << " " << dist[s][t].lim << endl;
        }
    }
