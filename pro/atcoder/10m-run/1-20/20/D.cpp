...
LL s[112345];
LL t[112345];
LL getLeft(LL *p, int n, LL x) { return *(upper_bound(p, p + n, x) - 1); }
LL getRight(LL *p, int n, LL x) { return *(lower_bound(p, p + n, x)); }

int main() {
    /* write here */
    int a, b, q;
    cin >> a >> b >> q;
    for (int i : range(a)) cin >> s[i + 2];
    for (int i : range(b)) cin >> t[i + 2];
    ...
    for (int _ : range(q)) {
        ...
        LL ret = 1e18;
        {
            LL p = getLeft(s, a, x);
            LL d = abs(p - x);
            chmin(ret, d + abs(p - getLeft(t, b, p)));
            chmin(ret, d + abs(p - getRight(t, b, p)));
        }
        {
            LL p = getRight(s, a, x);
            LL d = abs(p - x);
...

        {
            LL p = getLeft(t, b, x);
            LL d = abs(p - x);
            chmin(ret, d + abs(p - getLeft(s, a, p)));
            chmin(ret, d + abs(p - getRight(s, a, p)));
        }
        {
            LL p = getRight(t, b, x);
            LL d = abs(p - x);
