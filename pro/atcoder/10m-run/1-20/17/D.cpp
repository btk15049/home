...
struct RectangleSum {
    T* val;
    int R, C;
    inline T& getref(const int r, const int c) { return val[r * C + c]; }
    RectangleSum(A grid, int row, int col,
        ...
        for (int r = 0; r < row; r++) {
            for (int c = 0; c < col; c++) {
                getref(r + 1, c + 1) = getref(r + 1, c) + getref(r, c + 1)
                                       + grid[r][c] - getref(r, c);
            }
        }
    }
    inline T get(int rb, int cb, int re, int ce) {
        return getref(re, ce) - getref(re, cb) - getref(rb, ce)
               + getref(rb, cb);
    }
    RectangleSum() {}
};
...
int main() {
    ...
    RectangleSum<int, int[512][512]> rec(grid, N + 1, N + 1);
    for (int i : range(Q)) {
        int l, r;
        cin >> l >> r;
        l--;
        cout << rec.get(l, l, r, r) << endl;
    }
    return 0;
}