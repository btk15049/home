...
inline std::vector<T> wrapGrid(std::vector<T> grid, U material) {
    std::vector<T> wrappedGrid(grid.size(), T(grid[0].size(), material));
...
template <typename T>
std::string join(std::vector<T> v, char delimiter) {
    std::stringstream ss;
    bool isFirst = true;
    for (auto& it : v) {
        if (!isFirst) {
...
void sync(vector<int> &v, int i, int c) {
    if (c == 0) {
        return;
    }
    if (i == (int)v.size()) {
...
int main() {
    ...
    while (v.back() <= (LL)1e15) {
        // cerr << v.back() << endl;

        for (int i : range(val.size())) {
            val[i] = calc(i, v.back() + 1);
        }
        int p = 16 * 9 - 1;
        for (int i : range(val.size())) {
            if (val[i] == -1) continue;
