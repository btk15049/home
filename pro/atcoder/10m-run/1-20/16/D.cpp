...
int main() {
    ...
    for (int i : range(s.size())) {
        if (s[i] == '?' || s[i] == 'B') {
            LL l = S[i] - S[0];
            LL r = S[s.size()] - S[i + 1];
            ret += ModInt(l * r) * (ModInt(3) ^ max(l + r - 2, 0ll));
            {
                LL la = A[i] - A[0];
                ret += ModInt(la * r) * (ModInt(3) ^ max(l + r - 1, 0ll));
            }
            {
                LL rc = C[s.size()] - C[i + 1];
                ret += ModInt(l * rc) * (ModInt(3) ^ max(l + r - 1, 0ll));
            }
            {
                LL la = A[i] - A[0];
                LL rc = C[s.size()] - C[i + 1];
                ret += ModInt(la * rc) * (ModInt(3) ^ max(l + r, 0ll));
            }
        }
    }
    cout << *ret << endl;
