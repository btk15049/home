...
int main() {
    ...
    auto base = nestingVector<LL>(M, 1 << M);
    for (auto &v : base) {
        for (auto &it : v) {
            it = -1e12;
        }
    ...
    for (int i : range(N)) {
        auto nx = dp;
        int color, point;
        tie(color, point) = m[b[i]];
        chmax(nx[color][1 << color], point);
        cerr << color << " " << point;
        for (int j : range(M)) {
            for (int k : range(1 << M)) {
                chmax(nx[color][k | (1 << color)],
                      dp[j][k] + point + (color == j ? Y : 0));
            }
        }
        swap(dp, nx);
    }
    LL ret = 0;
    for (int j : range(M)) {
        for (int k : range((1 << M) - 1)) {
            chmax(ret, dp[j][k]);
        }
        chmax(ret, dp[j].back() + Z);
    }
    cout << ret << endl;
    return 0;
}