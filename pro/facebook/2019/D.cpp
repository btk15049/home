...
template <typename T>
void _topological_sort(int v, Graph<T>& g, vector<int>& vs, vector<int>& used,
                       int& id) {
    if (used[v]) return;
    used[v] = 1;
    for (int u : g.Ns(v)) {
        _topological_sort(u, g, vs, used, id);
    }
    vs[--id] = v;
}
template <typename T>
vector<int> topological_sort(Graph<T>& g) {
    ...
    for (int i : range(g.size())) {
        _topological_sort(i, g, vs, used, id);
    }
    return vs;
}

namespace _DSU {
    constexpr int BUF = 200;
    int ptr           = 0;
    int mem[BUF];
    int* get(int size) {
        ptr += size;
...
BlockBuffer(int, 5, 200, blocks);
void solve() {
    int N, M;
    cin >> N >> M;
    vector<int> X(M), Y(M), Z(M), P(N, 0);
    Graph<> g;
    string sp(N, ' ');
    sp.back() = '\n';
    ...
    for (int v : order) {
        for (int i : range(M)) {
            if (Z[i] == v) {
                if (X[i] != v) {
                    const int u = uf.find(X[i]);
                    if(uf.unite(v, u)){
                        P[u]        = v + 1;
                    }
                }
                if (Y[i] != v) {
                    const int u = uf.find(Y[i]);
                    if(uf.unite(v, u)){
                        P[u]        = v + 1;
                    }
                }
    ...
    for (int i : range(N)) {
        if (uf.same(0, i) == false) {
            const int v = uf.find(0);
            const int u = uf.find(i);
            if(uf.unite(v, u)){
                P[u] = v + 1;
            }
        }
    ...
    */
    ...
    Tree<> t(actual,root);
    HeavyLightDecomposition<Edge> hld(t, blocks);
    for (int i : range(M)) {
        if (hld.lca(X[i], Y[i]) != Z[i]) {
            cout << "Impossible\n";
            return;
        }
    }
