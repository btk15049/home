...
class SCC {
  ...
  private:
    ...
    void decomposition() {
        ...
        edge.resize(k);
        REP(v, size) {
            node[comp[v]].push_back(v);
            for (auto& u : e[v]) {
                if (!isSame(v, u)) {
                    edge[comp[v]].push_back(comp[u]);
                }
            }
        }
        size = k;
  ...
  public:
    bool isSame(int a, int b) { return comp[a] == comp[b]; }
    SCC(Graph& in) {
        size = in.size();
        e    = in;
        r_e.resize(size);
        REP(v, size) ITR(u, e[v]) r_e[u].push_back(v);
        decomposition();
    }
};
void dfs1(int v, V& used, VV& in) {
    if (used[v]) return;
    used[v] = 1;
    for (int u : in[v]) {
        dfs1(u, used, in);
...
/*
void dfs2(int v, V& used, V& vs, VV& in) {
    if (used[v]) return;
    used[v] = 1;
    for (int u : in[v]) {
        dfs2(u, used,vs, in);
...
void solve() {
    int N;
    cin >> N;
    Graph in(N), g1(N), g2(N);
    vector<ModInt> gram(N);
    for (int i : range(N)) {
        int a, b;
        cin >> a >> b;
        a--;
    ...
    SCC scc(g2);
    //cerr << "comp[0] = " << scc.comp[0] << endl; 
    {
        for (int i : range(scc.node.size())) {
            int e=0;
            for(int v:scc.node[i]){
                for(int u:g2[v]){
                    if(scc.comp[u]==scc.comp[v])e++;
                }
            }
            if(e>(int)scc.node[i].size()){
                //cerr << i << endl;
                cout << "UNBOUNDED" << endl;
                return;
            }
            if(e==(int)scc.node[i].size() && scc.comp[0]!=i){
                cerr << i;
                cout << "UNBOUNDED" << endl;
                return;
            }
