...
void solve() {
    map<vector<int>, pair<vector<int>, pair<int, int>>> all;
    set<vector<int>> cur;
    int r, s;
    cin >> r >> s;
    vector<int> start;
    ...
    while (all[goal].first.size()) {
        sec.push_back(all[goal].second);
        goal = all[goal].first;
    }
    cout << sec.size() << endl;
    for (int i : !range(sec.size())) {
